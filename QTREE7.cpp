...
struct node{
    ...
    static node *NIL; node *c[2], *p; multiset<int> s;
    bool r0; int w0, w1;
...
#define py y->p
    ...
    int sgn(){return p->l==this?0:p->r==this?1:-1;}
    void setc(int d,node*x){c[d]=x,px=this;}
    ...
    node* acs(){
        node *x = this, *y = NIL; do{
            x->splay();
            if (y != NIL) x->s.erase(x->s.find(y->w1));
            if (rx != NIL) x->s.insert(rx->w1);
            rx = y, x->upd();
            y = x, x = px;
        } while (x != NIL);
        return splay();
    ...
    int query(){
        node *x = rt()->r; //for(; x->rls(), lx != NIL; x = lx);
        return x->w1;
    }
...
} *NIL, *T[2][N]; int col[N], fa[N];
#define TT(u) T[col[u]][u]

int hd[N], suc[M], to[M], n;
#define aa to[i^1]
#define bb to[i]
#define v bb

inline void dfs(int u){
    REP_G(i, u) if (TT(v)->p == NIL){
        TT(v)->p = T[col[v]][fa[v] = u], dfs(v);
        T[col[v]][u]->s.insert(TT(v)->w1);
    }
    TT(u)->upd();
}
...
inline void Toggle(int u){
    TT(u)->cut(); col[u] ^= 1;
    TT(u)->link(T[col[u]][fa[u]]);
}

inline void Modify(int u, int w){
    T[0][u]->modify(w), T[1][u]->modify(w);
}
...
int main(){
    ...
    REP(i, N) T[0][i] = new node(), T[1][i] = new node();

    while (~scanf("%d", &n)){

        FOR_1(i, 0, n) T[0][i]->reset(), T[1][i]->reset();

        RST(hd); FOR_C(i, 2, n << 1){
            RD(aa, bb);
            suc[i] = hd[aa], hd[aa] = i++;
            suc[i] = hd[aa], hd[aa] = i;
        }
        ...
        REP_1(i, n) RD(col[i]);
        REP_1(i, n) T[1][i]->w0 = RDD(T[0][i]->w0);

        TT(1)->p = T[col[1]][0]; dfs(1);
        T[col[1]][0]->s.insert(TT(1)->w1);
